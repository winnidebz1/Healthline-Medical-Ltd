...
const Hero: React.FC<HeroProps> = ({ onBookClick }) => {
  ...
  return (
    <div className="relative overflow-hidden bg-white h-[500px] md:h-[600px] group">
      ...
      <button
        onClick={prevSlide}
        className="absolute left-4 top-1/2 -translate-y-1/2 z-30 p-2 rounded-full bg-white/50 backdrop-blur-md text-gray-800 hover:bg-white transition-all shadow-sm opacity-0 group-hover:opacity-100"
      >
        <ChevronLeft size={24} />
      </button>
      <button
        onClick={nextSlide}
        className="absolute right-4 top-1/2 -translate-y-1/2 z-30 p-2 rounded-full bg-white/50 backdrop-blur-md text-gray-800 hover:bg-white transition-all shadow-sm opacity-0 group-hover:opacity-100"
      >
